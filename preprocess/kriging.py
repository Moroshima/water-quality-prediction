...
draw = False  # 切换至图例绘制功能
save_picture_with_date_water_mark = False  # FOR DEBUG 是否在图片上添加时间水印
...
if not draw:
    ...
    for specific_date in date_array:
        ...
        for index, feature in enumerate(features):
            ...
            # FOR DEBUG: 绘制时间
            if save_picture_with_date_water_mark:
                draw = ImageDraw.Draw(img)
                draw.text(
                    (0, 0),
                    f"{pd.to_datetime(str(specific_date)).strftime("%Y%m%d%H")}",
                    fill=255,
                )
